...
local function loadHandlers()
	for _, moduleScript in handlersFolder:GetChildren() do
		...
		if typeof(handler) ~= "table" or typeof(handler.Type) ~= "string" then
			warn(moduleScript.Name .. " must return a table with a string Type field")
			continue
		end

		if typeof(handler.Interact) ~= "function" then
			warn(moduleScript.Name .. " must contain an Interact function")
			continue
		end
...
local function isPlayerCloseEnough(player, object, prompt)
...
local function setupInteractable(object)
	...
	if not object:IsA("BasePart") then
		warn("Interactable tag must be placed on a BasePart:", object:GetFullName())
		return
	end
	...
	connections[object] = prompt.Triggered:Connect(function(player)
		if not isPlayerCloseEnough(player, object, prompt) then
			return
		end
		...
		if handler.CanInteract then
			local canInteract = handler.CanInteract(player, object)
...
for _, object in CollectionService:GetTagged(INTERACTABLE_TAG) do
	setupInteractable(object)
end

CollectionService:GetInstanceAddedSignal(INTERACTABLE_TAG):Connect(setupInteractable)
